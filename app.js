...
(function () {
  ...
  var T = {
    en: {
      ...
      giftAll: 'Add gift box & card to my order · +{p}', giftAllOn: 'Gift box & card added to my order ✓',
      ...
      q6: 'Are the photos real?', a6: 'The items shown now are samples. Real photos of each misbah are being added.',
      ...
      copy: '© 2026 Boshehri Amber · Kuwait', demo: 'Demo website — front end only',
      ...
      total: 'Total', checkout: 'Checkout', cartNote: 'Demo checkout: your order opens as a summary you can send on WhatsApp. No payment is taken online.',
      ...
      loginHint: 'Members see their cart, favorites and orders in one place.', demoB: 'Demo only. Do not use a real password.',
      demoT: 'This is a front-end mockup: any email and any password will open the members area. Nothing you type is saved or sent anywhere.',
      email: 'Email', password: 'Password', phEmail: 'you@example.com', phPass: 'anything works in the demo', err: 'Please type an email and a password.', enter: 'Enter the members area',
      ...
      acctNote: 'Demo: you were let in with any email and password. The name above is taken from the email you typed and is kept only in this browser tab.',
      ...
      orderNote: 'Demo: the order is sent as a WhatsApp message to the shop. Payment and delivery are agreed there.', orderEmpty: 'Your cart is empty — add a misbah first.', inKwd: 'in KWD',
    ...
    ar: {
      ...
      giftAll: 'أضف علبة هدية وكرت لطلبي · +{p}', giftAllOn: 'أُضيفت علبة الهدية والكرت لطلبي ✓',
      ...
      q6: 'هل الصور حقيقية؟', a6: 'العناصر المعروضة حالياً تجريبية. تتم إضافة صور حقيقية لكل مسبحة.',
      ...
      copy: '© 2026 Boshehri Amber · الكويت', demo: 'موقع تجريبي — واجهة فقط',
      ...
      total: 'الإجمالي', checkout: 'إتمام الطلب', cartNote: 'إتمام تجريبي: يظهر طلبك كملخص يمكنك إرساله عبر واتساب. لا يُدفع أي مبلغ عبر الموقع.',
      ...
      loginHint: 'يرى الأعضاء سلتهم ومفضلتهم وطلباتهم في مكان واحد.', demoB: 'للتجربة فقط. لا تستخدم كلمة مرور حقيقية.',
      demoT: 'هذا نموذج واجهة فقط: أي بريد وأي كلمة مرور سيفتحان منطقة الأعضاء. لا يُحفظ أو يُرسل ما تكتبه إلى أي مكان.',
      email: 'البريد الإلكتروني', password: 'كلمة المرور', phEmail: 'you@example.com', phPass: 'أي شيء يعمل في التجربة', err: 'الرجاء كتابة بريد إلكتروني وكلمة مرور.', enter: 'ادخل منطقة الأعضاء',
      ...
      acctNote: 'تجريبي: تم إدخالك بأي بريد وكلمة مرور. الاسم أعلاه مأخوذ من البريد الذي كتبته ويُحفظ في هذا التبويب فقط.',
      ...
      orderNote: 'تجريبي: يُرسل الطلب كرسالة واتساب إلى المتجر. يُتفق على الدفع والتوصيل هناك.', orderEmpty: 'سلتك فارغة — أضف مسبحة أولاً.', inKwd: 'بالدينار الكويتي',
  ...
  function moneyKwd(kwd) { var s = kwd.toFixed(3).replace(/\.?0+$/, ''); return s + ' ' + (lang === 'ar' ? 'د.ك' : 'KWD'); }
  function giftP() { return money(GIFT, { plain: true }); }

  /* ---------- sample catalog (placeholders until real stock is added) ---------- */
  var ITEMS = [];
  var prices = [45, 120, 38, 260, 75, 480, 32, 150, 95, 210, 58, 330, 42, 180, 68, 400, 29, 135, 88, 240, 52, 310, 36, 165];
  var beadsArr = [33, 99, 33, 66, 33, 99, 33, 66, 99, 33, 33, 99, 66, 33, 33, 99, 33, 66, 33, 99, 33, 66, 33, 99];
  var sizes = [8, 10, 8, 12, 10, 12, 8, 10, 8, 12, 8, 10, 10, 12, 8, 12, 8, 10, 8, 12, 8, 10, 8, 12];
  for (var i = 0; i < 24; i++) {
    var d = new Date(2026, 8, 8); d.setDate(d.getDate() - i * 3);
    ITEMS.push({ id: 'm' + (i + 1), no: String(i + 1).padStart(2, '0'), price: prices[i], added: d.getTime(), isNew: i < 4, beads: beadsArr[i], size: sizes[i] });
  }
  function nameOf(it) { return t('item') + ' ' + it.no; }
  ...
  for (var k0 in cart) if (!byId(k0)) delete cart[k0];
  ...
  function cartTotal() { return itemsTotal() + giftTotal(); }
  ...
  function applyLang() {
    ...
    var tt = document.title; if (page) { var map = { home: 'Boshehri Amber', login: t('login') + ' · Boshehri Amber', dashboard: t('account') + ' · Boshehri Amber', members: t('cartfav') + ' · Boshehri Amber', order: t('order') + ' · Boshehri Amber' }; if (map[page]) document.title = map[page]; }
  }
  function setLang(l) { lang = l; save('ba-lang', l); applyLang(); refresh(); }
  function setCur(c) { if (!CUR[c]) return; cur = c; save('ba-cur', c); applyLang(); refresh(); }
  function setTheme(th) { theme = th; save('ba-theme', th); document.documentElement.setAttribute('data-theme', th); applyLang(); var m = $('meta[name=theme-color]'); if (m) m.setAttribute('content', th === 'light' ? '#F6F2EA' : '#0C0C0C'); }
  ...
  function card(it, idx) {
    ...
    return '<article class="item" data-id="' + it.id + '" style="animation-delay:' + ((idx || 0) * 45) + 'ms">' +
      '<div class="ph" data-view="' + it.id + '">' + ICO.bead + '<small>' + t('photo') + '</small>' + (it.isNew ? '<span class="new">' + t('newTag') + '</span>' : '') +
      ...
      '<div class="info"><h3 data-view="' + it.id + '">' + nameOf(it) + '</h3><div class="spec">' + t('beads', { n: it.beads }) + ' · ' + t('beadSize', { n: it.size }) + '</div><div class="price">' + money(it.price) + '</div>' +
      '<div class="acts"><button class="btn sm gold" data-add="' + it.id + '">' + (inCart ? t('added') : t('add')) + '</button>' +
      '<button class="gift-b' + (gift ? ' on' : '') + '" data-gift="' + it.id + '">🎁 ' + (gift ? t('giftOn', { p: giftP() }) : t('gift', { p: giftP() })) + '</button></div></div></article>';
  }
  ...
  function renderGrid(opts) {
    ...
    if (opts.skeleton) { grid.innerHTML = skeletons(slice.length); skTimer = setTimeout(paint, 420); } else paint();
  ...
  function openItem(id) {
    ...
    $('#pm-body').innerHTML =
      '<div class="pm-ph">' + ICO.bead + '<small>' + t('photo') + '</small>' + (it.isNew ? '<span class="new">' + t('newTag') + '</span>' : '') + '</div>' +
      '<div class="pm-info"><span class="tag">' + t('sample') + '</span><h2>' + nameOf(it) + '</h2><div class="price big">' + money(it.price) + '</div>' +
      (cur !== 'KWD' ? '<div class="mini">' + moneyKwd(it.price) + ' · ' + t('priceNote') + '</div>' : '') +
      '<dl class="specs"><div><dt>' + t('beadsL') + '</dt><dd>' + it.beads + '</dd></div><div><dt>' + t('sizeL') + '</dt><dd>' + it.size + ' mm</dd></div></dl>' +
      '<p class="desc">' + t('desc') + '</p>' +
      '<div class="acts"><button class="btn gold" data-add="' + id + '">' + (inCart ? t('added') : t('add')) + '</button>' +
      '<button class="gift-b' + (gift ? ' on' : '') + '" data-gift="' + id + '">🎁 ' + (gift ? t('giftOn', { p: giftP() }) : t('gift', { p: giftP() })) + '</button></div>' +
  ...
  document.addEventListener('click', function (e) {
    ...
    if (tgt.hasAttribute('data-add')) { cart[id] = cart[id] || { qty: 0, gift: false }; cart[id].qty += 1; save('ba-cart', cart); toast(t('addedToast')); }
    if (tgt.hasAttribute('data-fav')) { var i = favs.indexOf(id); if (i > -1) { favs.splice(i, 1); toast(t('favOff')); } else { favs.push(id); toast(t('favOn')); } save('ba-favs', favs); }
    if (tgt.hasAttribute('data-gift')) { cart[id] = cart[id] || { qty: 1, gift: false }; cart[id].gift = !cart[id].gift; save('ba-cart', cart); }
    if (tgt.hasAttribute('data-remove')) { delete cart[id]; save('ba-cart', cart); }
    if (tgt.hasAttribute('data-qty')) { var d = parseInt(tgt.getAttribute('data-d'), 10); if (cart[id]) { cart[id].qty += d; if (cart[id].qty <= 0) delete cart[id]; save('ba-cart', cart); } }
    if (tgt.hasAttribute('data-giftall')) { giftAll = !giftAll; save('ba-giftall', giftAll); }
    refresh();
  ...
  function cartLines() {
    ...
    var h = ids.map(function (id) { var it = byId(id), c = cart[id];
      return '<div class="line"><div class="ph" data-view="' + id + '">' + ICO.bead + '</div><div><b>' + nameOf(it) + '</b><div class="meta">' + money(it.price) + (c.gift ? ' · 🎁 +' + money(GIFT * c.qty) : '') + '</div>' +
  ...
  function renderFavs() {
    ...
    $$('.fav-lines').forEach(function (box) { box.innerHTML = favs.length ? favs.map(function (id) { var it = byId(id); return '<div class="line"><div class="ph" data-view="' + id + '">' + ICO.bead + '</div><div><b>' + nameOf(it) + '</b><div class="meta">' + money(it.price) + '</div></div><div class="acts"><button class="btn sm gold" data-add="' + id + '">' + t('add') + '</button><button class="fav on" data-fav="' + id + '" aria-label="' + t('remove') + '">' + ICO.heart + '</button></div></div>'; }).join('') : '<div class="empty">' + t('favEmpty') + '</div>'; });
  ...
  /* ---------- session (demo login) ---------- */
  function who() { try { return sessionStorage.getItem('ba-name'); } catch (x) { return null; } }
  $$('[data-checkout]').forEach(function (co) { co.addEventListener('click', function (e) { e.preventDefault(); if (!cartCount()) { toast(t('orderEmpty')); return; } location.href = who() ? 'order.html' : 'login.html?next=order'; }); });

  var form = $('#login-form');
  if (form) form.addEventListener('submit', function (e) {
    e.preventDefault();
    var email = form.email.value.trim(), pass = form.password.value, err = $('#err');
    if (!email || !pass) { err.style.display = 'block'; return; }
    var name = email.split('@')[0].replace(/[._-]+/g, ' ').trim(); name = name ? name.charAt(0).toUpperCase() + name.slice(1) : 'Member';
    try { sessionStorage.setItem('ba-name', name); } catch (x) {}
    form.password.value = '';
  ...
  function renderMembers() {
    ...
    var lo = load('ba-last-order', null), box = $('#last-order');
    if (box) box.innerHTML = lo ? '<span class="tag">' + t('orderNo') + ' ' + lo.no + '</span> <span class="mini">' + new Date(lo.at).toLocaleDateString(lang === 'ar' ? 'ar-KW' : 'en-GB') + ' · ' + moneyKwd(lo.total) + '</span>' : '';
  }
  if (GATED) {
    var nm = who();
    if (!nm) { location.replace('login.html' + (page === 'order' ? '?next=order' : '')); return; }
    ...
    var out = $('#logout'); if (out) out.addEventListener('click', function (e) { e.preventDefault(); try { sessionStorage.removeItem('ba-name'); } catch (x) {} location.href = 'index.html'; });
  }
  var lg = $('#login-link'); if (lg && who()) { lg.setAttribute('href', 'dashboard.html'); lg.setAttribute('data-i18n-aria', 'account'); }
  $$('[data-login-menu]').forEach(function (a) { if (who()) { a.setAttribute('href', 'dashboard.html'); a.setAttribute('data-i18n', 'account'); } });
  ...
  function renderOrder() {
    ...
    if (!cartCount() && lo && lo.sent) { // show the receipt of the order that was just sent
      box.innerHTML = '<div class="sent">✓</div><h2>' + t('sent') + '</h2><p class="mini"><span class="tag">' + t('orderNo') + ' ' + lo.no + '</span> · ' + moneyKwd(lo.total) + '</p>' +
        '<div class="receipt">' + lo.lines.map(function (l) { return '<div class="rl"><span>' + l[0] + '</span><span>' + l[1] + '</span></div>'; }).join('') + '</div>' +
    ...
    var nm = who() || 'Member';
    box.innerHTML = '<div class="row"><span class="tag">' + t('orderNo') + ' ' + orderNo + '</span><span class="mini">' + t('name') + ': ' + nm + '</span></div>' +
    ...
    $('#send-wa').addEventListener('click', function () {
      var lines = []; for (var id in cart) lines.push([nameOf(byId(id)) + ' ×' + cart[id].qty + (cart[id].gift ? ' 🎁' : ''), moneyKwd(byId(id).price * cart[id].qty)]);
      if (giftCount()) lines.push(['🎁 ' + t('giftLine', { n: giftCount() }), moneyKwd(giftTotal())]);
      lines.push([t('total'), moneyKwd(cartTotal())]);
      save('ba-last-order', { no: orderNo, at: Date.now(), total: cartTotal(), lines: lines, sent: true });
      cart = {}; giftAll = false; save('ba-cart', cart); save('ba-giftall', false);
  ...
  /* ---------- install as app ---------- */
  var deferred = null;
  window.addEventListener('beforeinstallprompt', function (e) { e.preventDefault(); deferred = e; $$('[data-install]').forEach(function (b) { b.hidden = false; }); });
  ...
  if (grid) { var m = location.search.match(/[?&]item=(m\d+)/); if (m) openItem(m[1]); }
